...
contract PokemonGame {
    ...
    modifier onlyOncePerQuarterMinute() {
        require(
            players[msg.sender].lastCatchTime + 15 seconds <= block.timestamp,
            "You can only perform this action once per 15 seconds!"
        );
        _;
    }

    event PokemonCatched(
        address indexed playerAddress,
        Pokemon indexed pokemonType
    );

    function catchPokemon(Pokemon _pokemonType)
        public
        onlyOncePerQuarterMinute
    {
        require(
            !players[msg.sender].pokemonsCatched[uint8(_pokemonType)],
            "You have already have this type of pokemon!"
        );
    ...
    function listCatchedPokemons(address _playerAddress)
        public
        view
    ...
    function listPokemonHolders(Pokemon _pokemonType)
        public
        view
